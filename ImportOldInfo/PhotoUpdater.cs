...
namespace ImportOldInfo
{
    ...
    public static class PhotoUpdater
    {
        ...
        /// <summary>
        /// Переносим изображения по старому идентификатору альбома
        /// </summary>
        /// <param name="org"></param>
        /// <param name="repository"></param>
        /// <param name="helper"></param>
        public static void PhotoUpdate(Org org, Repository repository, ParamsHelper helper)
        {
            // фотоальбомы
            IEnumerable<PhotoAlbumNew> newAlbums = repository.GetNewAlbumsWithPath(org);

            // текущий номер альбома
            int currentAlbumNumber = 0;
            ...
            foreach (var album in newAlbums)
            {
                ...
                {
                    // путь до старой директории
                    string oldDirectory = ($"{helper.OldDirectory}{album.Org}{album.OldPath.Replace("./", "/")}").Replace("/", "\\");
                    ...
                    if (Directory.Exists(oldDirectory))
                    {
                        ...
                        if (fi != null && fi.Count() > 0)
                        {
                            string datePath = $"/{album.Date.ToString("yyyy_MM")}/{album.Date.ToString("dd")}/";
